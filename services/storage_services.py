...
class StorageService:
    ...
    def save_hubspot_payload(self, data: Dict[str, Any], meeting_id: Optional[str] = None) -> str:
        ...
        self.db.storage.from_(BUCKET).upload(filename, json.dumps(data).encode(), {"content-type": "application/json"})
        return filename
    ...
    def save_email_draft(self, data: Dict[str, Any], meeting_id: Optional[str] = None) -> str:
        ...
        self.db.storage.from_(BUCKET).upload(filename, body.encode(), {"content-type": "text/plain"})
        return filename
